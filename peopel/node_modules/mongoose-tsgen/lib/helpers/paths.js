...
exports.getConfigFromFile = (configPath) => {
...
exports.getModelsPaths = (basePath, extension = "ts") => {
    let modelsPaths;
    if (basePath && basePath !== "") {
        ...
        const modelsFolderPath = ext === "" ? path_1.default.join(basePath, `!(index).${extension}`) : basePath;
        ...
        if (modelsPaths.length === 0) {
            throw new Error(`Recursive search could not find any model files at "**/models/*.${extension}". Please provide a path to your models folder.`);
        }
    }
    return modelsPaths.map((filename) => path_1.default.join(process.cwd(), filename));
};
exports.cleanOutputPath = (outputPath) => {
